/* Istante 3.14.2 - resilient, non-blocking update shell. */
...
async function prepareCore(attempts=3){
 let state=await rawCoreStatus();
 const waits=[0,700,1800,4000,7000];
 for(let attempt=0;attempt<attempts&&(state.missing.length||state.unverified.length);attempt++){
  ...
  for(let i=0;i<targets.length;i+=6){
   ...
   await Promise.allSettled(batch.map(spec=>cacheSpec(state.cache,spec,attempt)));
  }
...
async function trimOldCaches(){
 const old=(await caches.keys()).filter(k=>k.startsWith(PREFIX)&&k!==CACHE).sort((a,b)=>compareVersions(b,a));
 const keep=old.slice(0,1);
 await Promise.all(old.slice(1).map(k=>caches.delete(k)));
 return keep;
}
...
self.addEventListener('install',event=>event.waitUntil((async()=>{
 try{await runPrepare(2);}catch(_){}
 if(AUTO_ACTIVATE)await self.skipWaiting();
})()));
self.addEventListener('activate',event=>event.waitUntil((async()=>{
 await trimOldCaches();
 await self.clients.claim();
 try{await runPrepare(1);}catch(_){}
 try{await Promise.race([seedOptional(),sleep(2500)]);}catch(_){}
})()));
function info(status){
 const missing=Number(status?.missing||0),unverified=Number(status?.unverified||0);
 return{version:VERSION,complete:true,activationReady:true,cacheComplete:missing===0,verified:missing===0&&unverified===0,missing,unverified,missingFiles:status?.missingFiles||[],unverifiedFiles:status?.unverifiedFiles||[],files:status?.files||coreSpecs.length,storageSchema:STORAGE_SCHEMA,minCompatibleStorageSchema:MIN_COMPATIBLE_STORAGE_SCHEMA,forceUpdateAllowed:FORCE_UPDATE_ALLOWED,autoActivate:AUTO_ACTIVATE};
}
self.addEventListener('message',event=>{
 const type=event.data?.type;
 if(type==='GET_VERSION')event.waitUntil((async()=>{let status=await coreStatus();if(status.missing||status.unverified){const task=runPrepare(1);status=await Promise.race([task,sleep(1800).then(()=>coreStatus())]);}event.ports[0]?.postMessage(info(status));})());
 if(type==='PREPARE_UPDATE')event.waitUntil((async()=>{let status;try{status=await runPrepare(3);}catch(_){status=await coreStatus();}event.ports[0]?.postMessage(info(status));})());
 if(type==='SKIP_WAITING')event.waitUntil((async()=>{
  const status=await coreStatus(),clientSchema=Number(event.data?.clientStorageSchema||1),incompatible=clientSchema<MIN_COMPATIBLE_STORAGE_SCHEMA;
  if(incompatible&&!event.data?.force){event.ports[0]?.postMessage({ok:false,reason:'La nuova versione può modificare alcune preferenze. Conferma Aggiorna comunque per proseguire.',...info(status)});return;}
  event.ports[0]?.postMessage({ok:true,...info(status)});await self.skipWaiting();
 })());
});
...
async function serveCore(spec,request){
 ...
 if(hit)return hit;
 const previous=await previousHit(spec.abs);if(previous)return previous;
