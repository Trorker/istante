...
(function(){
 ...
 function create({getSettings,getDraft,radio,notify,openTimer,icon}){
  ...
  function describe(){const s=getSettings(),label=s.timerAction==='radio'&&s.radioEnabled?'Alla fine: '+radio.station().name:s.timerAction==='silent'?'Alla fine: un piccolo avviso, senza suono.':'Alla fine: '+({chime:'piccoli rintocchi',bell:'campana morbida',pulse:'segnale delicato'}[s.timerSound])+(s.timerVolume===0?' (volume a zero).':'.');$('#timer-finish-description').textContent=label;}
  ...
  function startTimer(){
   ...
   if(state.state==='paused'){void prepare();state.deadline=Date.now()+state.remaining;state.state='running';save();render();return;}
   ...
   $('#timer-error').hidden=true;clearSound();result('');alarmRadio=false;void prepare();state={state:'running',duration,remaining:duration,deadline:Date.now()+duration,id:Date.now().toString(36)+Math.random().toString(36).slice(2,6)};restored=false;save();render();M.flash($('#timer-running'));notify('Il tuo momento \u00e8 iniziato.');
  }
  function pauseTimer(){if(state.state!=='running')return;const left=T.remaining(state);if(left<=0){finish();return;}state.remaining=left;state.state='paused';save();render();}
  function reset(){clearSound();alarmRadio=false;state={state:'idle',duration:getSettings().timerMinutes*60000,remaining:getSettings().timerMinutes*60000,deadline:0,id:''};save();result('');$('#timer-error').hidden=true;setDuration(state.duration);render();}
  function finish(silentRestore=false){
   if(state.state!=='running')return;state.state='done';state.remaining=0;save();render();
  ...
  return{apply,open,prepare,clearSound,inspect:()=>({timer:{...state},armed,attempted,window:previousWindow}),destroy(){destroyed=true;clearInterval(interval);clearSound();}};
 }
