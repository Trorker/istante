...
(function(){
 ...
 function create({getSettings,getDraft,radio,ambient,notify,openTimer,icon}){
  ...
  function setDuration(ms){
   ...
   const dial=$('#timer-duration-dial'),pointer=$('#timer-dial-pointer'),dialMinutes=Math.max(1,Math.min(60,minutes));if(dial){dial.setAttribute('aria-valuenow',String(dialMinutes));dial.setAttribute('aria-valuetext',dialMinutes+' '+(dialMinutes===1?'minuto':'minuti'));dial.dataset.minutes=String(dialMinutes);}if(pointer)pointer.style.setProperty('--timer-select-angle',(dialMinutes%60)*6+'deg');document.querySelectorAll('.timer-dial-mark').forEach(mark=>mark.classList.toggle('active',Number(mark.dataset.minute)===dialMinutes));
  ...
  function render(){
   ...
   const progress=state.duration?Math.max(0,Math.min(100,(1-ms/state.duration)*100)):0;const orbit=$('#timer-orbit');if(orbit){const elapsed=state.duration?Math.max(0,state.duration-ms):0;orbit.setAttribute('aria-valuenow',String(Math.round(progress)));orbit.style.setProperty('--timer-progress',progress+'%');orbit.style.setProperty('--timer-second-angle',(elapsed/60000*360)+'deg');orbit.dataset.state=state.state;}const modalClock=$('#timer-modal-clock');if(modalClock)modalClock.textContent=T.formatTime(Date.now(),s.timeFormat);
