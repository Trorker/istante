...
(function(){
 ...
 function create({getSettings,getDraft,radio,ambient,notify,openTimer,icon}){
  ...
  function setDuration(ms){
   ...
   document.querySelectorAll('[data-duration]').forEach(b=>b.classList.toggle('active',Number(b.dataset.duration)*60000===ms));document.querySelectorAll('[data-page-duration]').forEach(b=>b.classList.toggle('active',Number(b.dataset.pageDuration)*60000===ms));
  }
  ...
  function renderPage(ms,progress){
   ...
   if(reset)reset.hidden=state.state==='idle';const quick=$('#timer-page-quick');if(quick)quick.hidden=state.state!=='idle';document.querySelectorAll('[data-page-duration]').forEach(b=>{b.disabled=state.state!=='idle';b.classList.toggle('active',state.state==='idle'&&Number(b.dataset.pageDuration)*60000===state.duration);});const nowClock=$('#timer-page-clock');if(nowClock)nowClock.textContent=T.formatTime(Date.now(),s.timeFormat);
  ...
  $('#timer-page-quick')?.addEventListener('click',event=>{const button=event.target.closest('[data-page-duration]');if(!button)return;event.preventDefault();choosePreset(button,'pageDuration');});
