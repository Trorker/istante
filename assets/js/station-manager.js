...
(function(){'use strict';
 function create({library,icon,notify}){
  ...
  function render(){
   const list=$('#station-manager-list'),q=$('#station-manager-search').value.trim().toLocaleLowerCase('it');list.replaceChildren();
   const stations=library.list().filter(s=>(!filter||library.favorite(s.id))&&s.name.toLocaleLowerCase('it').includes(q));
   for(const s of stations){
    const row=node('article','station-manager-row'),title=node('div','station-manager-title');title.append(node('strong',null,s.name),node('small',null,s.custom?'Aggiunta da te':s.provider));row.append(title);
