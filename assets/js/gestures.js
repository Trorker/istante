...
(function(){'use strict';
 ...
 function blocked(target){return !!target.closest?.('dialog[open],#radio-panel,input,select,textarea,[contenteditable=true],button,a,.control-popup,.bottom-bar,.view-dots,.calendar-toolbar,.topbar');}
 ...
 document.addEventListener('pointerup',e=>{
  ...
  if(elapsed<=760&&Math.abs(dx)<40&&Math.abs(dy)<40){const now=Date.now();if(lastTap&&now-lastTap<=760&&Math.hypot(e.clientX-lastTapX,e.clientY-lastTapY)<=88){e.preventDefault();resetTap();toggle();return;}lastTap=now;lastTapX=e.clientX;lastTapY=e.clientY;return;}
  resetTap();if(elapsed>1900||Math.abs(dy)<44||Math.abs(dy)<Math.abs(dx)*1.04)return;const steps=Math.max(1,Math.min(5,Math.round(Math.abs(dy)/58))),delta=(dy<0?1:-1)*steps*5;document.dispatchEvent(new CustomEvent('istante:volume-gesture',{detail:{delta}}));
 },{passive:false,capture:true});
 document.addEventListener('pointercancel',()=>{start=null;resetTap();},{passive:true,capture:true});
 document.addEventListener('dblclick',e=>{const s=read();if(!s.audioVolumeGesture||blocked(e.target)||!inZone(e.clientX,e.clientY))return;e.preventDefault();resetTap();toggle();},{capture:true});
})();
