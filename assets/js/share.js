...
(function(){'use strict';
 function create({getSnapshot,open,notify}){
  ...
  async function copy(){
   try{if(!navigator.clipboard)throw Error('clipboard');await navigator.clipboard.writeText(card.URL);notify('Link di Istante copiato.');}
   catch(_){const area=document.createElement('textarea');area.value=card.URL;area.style.cssText='position:fixed;opacity:0;left:0;top:0';document.body.append(area);area.select();let ok=false;try{ok=document.execCommand('copy');}catch(_){}area.remove();notify(ok?'Link di Istante copiato.':'Link: '+card.URL);}
  }
  function link(){
