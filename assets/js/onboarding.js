...
(function(){
 ...
 function create({store,open}){
  ...
  const definitions=[
   ...
   {target:'#settings-open',note:'quasi tutto può diventare tuo',title:'Personalizza quasi ogni cosa.',copy:'Dalle Impostazioni puoi personalizzare quasi ogni parte di Istante: aspetto, caratteri, cielo, meteo, calendario, frasi, audio, timer e comportamento dello screensaver.'},
   ...
   {virtualTarget:'audio-zone',touchOnly:true,note:'gesti pensati per il touch',title:'Questa è l’area dei comandi touch.',copy:'La zona tratteggiata corrisponde davvero al lato destro usato dalle gesture: doppio tap per Play/Pausa e swipe verticale per cambiare il volume di 10. Nel Calendario queste gesture audio sono disattivate.'},
   {view:'calendar',calendarView:'year',target:'.calendar-views',fallback:'#calendar-view',note:'scegli quanto vicino guardare',title:'Il Calendario cambia prospettiva.',copy:'Entrando qui parti subito dal Calendario. Anno, Mese, Settimana, Giorno e Agenda mostrano gli stessi calendari con livelli di dettaglio diversi.'},
  ...
  function simulateWeather(){
   if(hasPlace())return;
   const env=byId('environment-line'),weather=byId('weather-line'),solar=byId('solar-line');
   if(!env||!weather||env.classList.contains('tour-simulated-weather'))return;
   ...
   if(byId('weather-text'))byId('weather-text').textContent='18°';
   if(byId('weather-condition'))byId('weather-condition').textContent='Sereno';
   if(byId('weather-place'))byId('weather-place').textContent='Esempio';
   if(byId('weather-age'))byId('weather-age').textContent='';
   if(byId('sunrise-time'))byId('sunrise-time').textContent='07:30';
   if(byId('sunset-time'))byId('sunset-time').textContent='18:45';
   if(byId('solar-note'))byId('solar-note').textContent='Simulazione';
  }
  ...
  function restoreSimulation(){
   for(const nodes of simulations){
    for(const snap of nodes){
     const el=snap.el;if(!el?.isConnected)continue;
     el.hidden=snap.hidden;el.textContent=snap.text;el.innerHTML=snap.html;
     if(snap.tooltip)el.dataset.istanteTooltip=snap.tooltip;else delete el.dataset.istanteTooltip;
     el.classList.remove('tour-simulated-weather','tour-simulated-event');
    }
   }
   simulations=[];
  }
  function renderLocationSetup(host){
   ...
   const p=store.read('place',null),box=document.createElement('div');box.className='tour-inline-setup tour-location-setup';
   if(p&&Number.isFinite(Number(p.lat))&&Number.isFinite(Number(p.lon))){
    box.innerHTML='<p class="field-note"><strong>Posizione già configurata.</strong> Istante userà la località salvata per cielo, alba, tramonto e meteo.</p>';host.append(box);return;
   }
   ...
   host.append(box);
   const status=box.querySelector('[data-location-status]'),locate=box.querySelector('[data-location-action=locate]');
   locate.addEventListener('click',()=>{
    locate.disabled=true;status.textContent='Attendo la posizione del dispositivo…';
    document.dispatchEvent(new CustomEvent('istante:onboarding-location-locate',{detail:{done(ok,message){
     locate.disabled=false;status.textContent=message||(ok?'Posizione salvata.':'Posizione non disponibile.');
     if(ok){locate.innerHTML='<span class="icon">'+window.IstanteIcons.render('check')+'</span>Posizione salvata';locate.setAttribute('aria-pressed','true');const place=byId('weather-place');if(place)place.textContent='La mia posizione';}
     queue();
    }}}));
   });
  ...
  function gestureZoneRect(){
   ...
   return{x:w-edge,y:top,width:edge,height:Math.max(40,h-top-bottom)};
  }
  function rawRect(){
   if(virtualTarget==='audio-zone')return gestureZoneRect();
   return target?.getBoundingClientRect?.()||null;
  }
  function position(rawOverride){
   raf=0;if(!dialog.open||(!target&&!virtualTarget))return;
   const W=window.visualViewport?.width||innerWidth,H=window.visualViewport?.height||innerHeight;
   byId('tour-sketch').setAttribute('viewBox',`0 0 ${W} ${H}`);
   const raw=rawOverride||rawRect();if(!raw)return;
   const margin=10,r={x:clamp(raw.x-6,4,W-4),y:clamp(raw.y-6,4,H-4),w:Math.min(raw.width+12,W-8),h:Math.min(raw.height+12,H-8)};
   r.w=Math.min(r.w,W-4-r.x);r.h=Math.min(r.h,H-4-r.y);
   const cardWidth=W<=740?Math.min(334,W-18):Math.min(380,W-28,H<450?Math.max(240,W*.46):380);
   card.style.width=cardWidth+'px';card.style.maxHeight=Math.max(130,H-(W<=740?14:28))+'px';
  ...
  function queue(){
   ...
   const sample=()=>{
    if(token!==positionToken||!dialog.open||(!target&&!virtualTarget))return;
    const raw=rawRect();if(!raw)return;
    const now=[raw.x,raw.y,raw.width,raw.height],delta=previous?Math.max(...now.map((v,i)=>Math.abs(v-previous[i]))):Infinity;
    stable=delta<.45?stable+1:0;previous=now;frames++;
    if(stable>=1||frames>=12){position(raw);return;}
    raf=requestAnimationFrame(sample);
   };
   raf=requestAnimationFrame(sample);
  }
  function showStep(){
   const d=steps[index];if(!d){M.dismiss(dialog);return;}
   setSidebarPreview(false);
   document.dispatchEvent(new CustomEvent('istante:onboarding-preview-view',{detail:{view:d.view||'dashboard'}}));
   if(d.calendarView)document.dispatchEvent(new CustomEvent('istante:onboarding-calendar-view',{detail:{view:d.calendarView}}));
   virtualTarget=d.virtualTarget||'';dialog.classList.toggle('tour-virtual-target',!!virtualTarget);
   target=virtualTarget?null:$(d.target);
   if(!virtualTarget&&!visible(target))target=$(d.fallback||'__unused__');
   if(!virtualTarget&&!visible(target))target=d.view==='calendar'?byId('calendar-view'):byId('app-shell');
   ...
   const choices=byId('tour-choices');choices.replaceChildren();choices.hidden=true;if(d.locationSetup)renderLocationSetup(choices);
   ...
   queue();byId('tour-next').focus({preventScroll:true});
  }
  function launchTour(){
   steps=selectSteps();index=0;if(!steps.length)return;
   originView=document.body.classList.contains('view-calendar')?'calendar':'dashboard';
   document.body.classList.add('is-touring');preparePresentation();open('tour');showStep();
  }
