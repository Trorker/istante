...
function create({getSettings,getSun,getWeather=()=>null,store,openWelcome}){
 ...
 const clouds=document.createElement('div');clouds.className='sky-weather-veil';sky.append(clouds);
 ...
 function update(now){
  ...
  document.documentElement.style.setProperty('--weather-orb',String(atmosphere.orb));sky.hidden=!s.celestialSky;sky.dataset.period=isDay?'day':'night';sky.classList.toggle('sky-static',!s.motion||reduced.matches||document.hidden);sky.style.setProperty('--sky-x',(9+82*progress)+'%');sky.style.setProperty('--sky-y',(20-13*sin(PI*progress))+'%');
  ...
  const sk=Math.floor(+now/10000)+':'+sky.clientWidth+':'+sky.clientHeight+':'+isDay+':'+atmosphere.kind;if(sk!==lastSky){lastSky=sk;drawStars(now,isDay);}
