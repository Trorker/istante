/* Istante custom tooltips. Browser-native title bubbles are suppressed globally. */
(function(){'use strict';
 ...
 function tooltipText(el){return (el?.dataset?.istanteTooltip||el?.getAttribute?.('title')||((el?.classList?.contains('icon-button')||el?.classList?.contains('collection-link'))?el?.getAttribute?.('aria-label'):'')||'').trim();}
 ...
 function reveal(el){if(!el?.isConnected||pending!==el)return;pending=null;const text=tooltipText(el);if(!text)return;active=el;const t=ensure();t.textContent=text;t.hidden=false;t.classList.remove('is-visible');cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{place(el);t.classList.add('is-visible');el.setAttribute('aria-describedby',t.id);});}
 function schedule(el,delay){const text=tooltipText(el);if(!text)return;absorb(el);clearTimeout(showTimer);if(active&&active!==el)hide(active);pending=el;showTimer=setTimeout(()=>reveal(el),delay);}
 function hide(el=active){clearTimeout(showTimer);showTimer=0;pending=null;if(!tip)return;if(active&&el&&el!==active)return;active?.removeAttribute('aria-describedby');active=null;tip.classList.remove('is-visible');setTimeout(()=>{if(tip&&!active&&!pending)tip.hidden=true;},130);}
 ...
 // Capture before the browser can keep a native title long enough to show it.
 ...
 document.addEventListener('close',()=>hide(),true);document.addEventListener('istante:view-change',()=>hide());document.addEventListener('keydown',e=>{if(e.key==='Escape')hide();});
